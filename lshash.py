...
class LSHash(object):
    ...
    def __init__(self, hash_size, input_dim, num_hashtables=1,
                 storage_config=None, matrices_filename=None, overwrite=False, max_queue_size=2):
        ...
        if storage_config is None:
            ...
            storage_config = {'queue': max_queue_size}
    ...
    def _as_np_array(self, json_or_tuple):
        ...
        if isinstance(tuples[0], tuple):
            ...
            #print "here"
            #print tuples[0][0].toarray()[0]
            
            return np.asarray(tuples[0][0].toarray()[0])
    ...
    #query to find the approx. nearest neighbor
    def arpoxNN(self, query_point, distance_func = None, L = 1000):
        candidate_set = dict()
        if not distance_func:
    ...
    def hash_dotproduct(x,y):
        ...
        a = set(x.indices)
        for i in a:
            p += x.data[np.where(x.indices == i)[0][0]]*y[i]*1.0
        return p
