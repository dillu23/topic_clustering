...
class onlineTfidfVectorizer(BaseEstimator, VectorizerMixin):
    """Convert a collection of text documents to a matrix of token counts
    ...
    'idf_' :  dict
        A mapping of terms to inverse frequency score
    
    'document_counts_' :  dict
        A mapping of terms to number of documents it occurs in.
    
    #TODO  sublinear_tf not implemented
    See also
    --------
    HashingVectorizer, TfidfVectorizer
    """
    #TODO look into the parameters
    def __init__(self, input='content', charset='utf-8',
                 charset_error='strict', strip_accents=None,
                 lowercase=True, preprocessor=None, tokenizer=None,
                 stop_words=None, token_pattern=ur"(?u)\b\w\w+\b",
        ...
        for i, term_count_dict in enumerate(term_count_dicts):
            for term, count in term_count_dict.iteritems():
                ...
                if j is not None:
                    ...
                    j_indices.append(j)
                    ct = count
                    if (self.binary and ct > 0):
                            ct = 1
                    if (self.sublinear_tf):
                        ct = np.log(ct) + 1
                    if(self.use_idf):
                        values.append(float(ct) * idf_scores.get(term))
                    else:
                        values.append(count)
            # free memory as we go
            term_count_dict.clear()
        shape = (i + 1, max(max(vocabulary.itervalues()) + 1, self.min_dict_size))
        spmatrix = sp.coo_matrix((values, (i_indices, j_indices)),
                                 shape=shape, dtype=self.dtype)
        if self.binary:
            spmatrix.data.fill(1)
        if self.norm:
            spmatrix = normalize(spmatrix, norm=self.norm, copy=False)
        return spmatrix.tocsr()
    ...
    def fit_transform(self, raw_documents, y=None):
        ...
        self.total_count = n_doc
        for keyword in vocab:
            idf_score[keyword] = np.log(float(n_doc + int(self.smooth_idf))/(document_counts[keyword] + int(self.smooth_idf))) + 1.0
        self.idf_ = idf_score
        # the term_counts and document_counts might be useful statistics, are
        # we really sure want we want to drop them? They take some memory but
        # can be useful for corpus introspection
    ...
    def transform(self, raw_documents):
        ...
        vectors : sparse matrix, [n_samples, n_features]
        """
        if not hasattr(self, 'vocabulary_') or len(self.vocabulary_) == 0 or len(self.idf_) ==0:
            raise ValueError("Vocabulary wasn't fitted or is empty!")
        ...
        # XXX @larsmans tried to parallelize the following loop with joblib.
        # The result was some 20% slower than the serial version.
        analyze = self.analyze2
        term_counts_per_doc = []
        vocab = self.vocabulary_
        document_count = Counter()
        for doc in raw_documents:
            term_count = Counter(analyze(doc))
            term_counts_per_doc.append(term_count)
            document_count.update(term_count.iterkeys())

        for term in set(document_count) - set(vocab):
            vocab[term] = len(vocab)
        self.vocabulary_ = vocab
        self.document_counts.update(document_count)
        self.total_count = self.total_count + len(term_counts_per_doc)
        for term in document_count:
            self.idf_[term] = np.log(float(self.total_count + int(self.smooth_idf))/(self.document_counts[term] + int(self.smooth_idf))) + 1.0
        return self._term_count_dicts_to_matrix(term_counts_per_doc)
...
class onlineTfidfVectorizer2(BaseEstimator, VectorizerMixin):
    """Convert a collection of text documents to a matrix of token counts
